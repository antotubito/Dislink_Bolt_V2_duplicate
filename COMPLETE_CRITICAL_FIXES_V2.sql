-- =====================================================
-- COMPLETE CRITICAL FIXES V2 - DISLINK SECURITY AUDIT
-- =====================================================
-- This script addresses all critical issues identified in the security audit
-- V2: Handles existing policies gracefully to avoid conflicts
...
-- Drop ALL possible existing policies for qr_scan_tracking to avoid conflicts
...
DROP POLICY IF EXISTS "Allow users to read their scan data" ON qr_scan_tracking;
DROP POLICY IF EXISTS "Users can view their own scan data" ON qr_scan_tracking;
DROP POLICY IF EXISTS "Users can read their own scan data" ON qr_scan_tracking;
...
-- QR Scan Tracking Policies - Drop all possible existing policies first
...
DROP POLICY IF EXISTS "Allow users to insert scan data" ON qr_scan_tracking;
DROP POLICY IF EXISTS "Users can insert their own scan data" ON qr_scan_tracking;

-- Create QR scan tracking policies
...
-- Connection Codes Policies - Drop all possible existing policies first
DROP POLICY IF EXISTS "Allow users to manage their own connection codes" ON connection_codes;
DROP POLICY IF EXISTS "Allow users to manage connection codes" ON connection_codes;
DROP POLICY IF EXISTS "Users can manage their own connection codes" ON connection_codes;
DROP POLICY IF EXISTS "Allow anonymous read of active connection codes" ON connection_codes;
DROP POLICY IF EXISTS "Allow anonymous read connection codes" ON connection_codes;

-- Create connection codes policies
...
-- Email Invitations Policies - Drop all possible existing policies first
DROP POLICY IF EXISTS "Allow sender to manage their invitations" ON email_invitations;
DROP POLICY IF EXISTS "Allow users to manage their invitations" ON email_invitations;
DROP POLICY IF EXISTS "Users can manage their invitations" ON email_invitations;
DROP POLICY IF EXISTS "Allow anonymous read for invitation validation" ON email_invitations;
DROP POLICY IF EXISTS "Allow anonymous read invitations" ON email_invitations;

-- Create email invitations policies
...
-- Connection Requests Policies - Drop all possible existing policies first
DROP POLICY IF EXISTS "Allow users to view their own requests" ON connection_requests;
DROP POLICY IF EXISTS "Allow users to view requests" ON connection_requests;
...
DROP POLICY IF EXISTS "Allow users to delete connection requests" ON connection_requests;

-- Create connection requests policies
...
SELECT 'Schema Consistency Fix Applied Successfully' as status;
